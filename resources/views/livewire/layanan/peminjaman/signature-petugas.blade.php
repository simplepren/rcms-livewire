...
class extends Component {
    ...
    {
        $data = User::where('enc_id', $id)->first();
        if($data){
            $this->id_user = $data->enc_id;
        }else{
            return abort(404);
        }
    }
    ...
    public function formEsign()
    {
        ...
        User::where('enc_id', $this->id_user)->update(['esign' => $file]);
        $this->dispatch('success',['message' => 'E-Signature user berhasil direkam.']);
        $this->kembali();
    }
...
<div class="">
    <div wire:loading wire:target="formEsign, kembali" class="fixed left-0 top-0 bg-white opacity-75 text-center w-full h-full" style="z-index: 51">
        ...
        <form wire:submit.prevent="formEsign">
            ...
            <div class="mt-4" style="width: 600px; margin: 0 auto">
                ...
                <div class="px-4 flex justify-start gap-2">
                    ...
                    <x-my-danger-button class="text-sm" id="clear">Clear</x-my-danger-button>
                    <x-my-warning-button wire:click="kembali" class="text-sm" id="clear">Back</x-my-warning-button>
                </div>
            </div>
        </form>
    </div>
